...
if(isset($_POST['payer']))
{
    ...
    {
        echo 'test';
        $bdd->query("INSERT INTO commande (user_id, montant, date)
         VALUES (" . $_SESSION['user']['id_user'] . ", " . montantTotal() . ", NOW())");

        // lastInsertID() : permet de récupérer la dernière clé primaire insérée en BDD, ici le dernier  id_commande afin e pouvoir l'insérer dans la 
        // table 'détails_commande' et de pouvoir relier chaque produit à la onne commande
         $idCommande = $bdd->lastInsertID(); 
        // La boucle FOR tourne autant de fois que nous avons de produit dans le tableau $_SESSION['panier']['id_article'] dans la session
         for($i = 0; $i < count($_SESSION['panier']['id_article']); $i++)
         {
            $bdd->query("INSERT INTO details_commande (commande_id, article_id, quantite, prix) VALUES($idCommande, " . $_SESSION['panier']['id_article'][$i] .
         ...
         // on stock dans la session l'id de la commande qui nous servira de num de commande pour l'internaute 
         $_SESSION['num_commande'] = $idCommande;
         // on vide le panier dans la session après la validation de la commande
         unset($_SESSION['panier']);
         // Une fois la commande validée, on redirige l'internaute vers la page validation 
        ...
        <?php for($i = 0; $i < count($_SESSION['panier']['id_article']); $i++): ?>
            ...
            <div class="container col-12 col-sm-12 col-md-12 col-lg-8 col-xl-8 mx-auto d-flex justify-content-center shadow-sm px-0 mb-2">
                ...
                <div class="col-md-7 bg-white d-flex flex-column justify-content-center p-2">
                    ...
                    <p class="mb-0"><a href="<?= URL ?>boutique.php" class="alert-link text-dark liens-supp-produit-panier">Supprimer
                     <?= deletePanier($_SESSION['panier']['id_article']);   ?></a></p>
                </div>
